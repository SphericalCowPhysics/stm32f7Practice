...
namespace F7SkeletonNS {
    public partial class MainForm : Form
    {
        F7SkeletonComms theBoard;
        int fwRev = 0;
        ...
        private void InitBoard()
        {
            try
            {
                ...
                fwRev = theBoard.GetReg(F7SkeletonComms.REGS.RegFirmWareVersion);
            }
            catch (Exception)
            {
                fwRev = 0;
        ...
        private void PopulatePorts()
        {
            ...
            List<string> portNames = F7SkeletonComms.GetPortNames();
        ...
        {
            tickChecked = false;
            for (F7SkeletonComms.REGS i = (F7SkeletonComms.REGS)0; i < F7SkeletonComms.REGS.RegLast; i++)
            {
                if (statusList.Rows[(int)i].Cells[0].Value.ToString() == true.ToString())
                {
                    logFile.Write(i.ToString() + ',');
                    if (i == F7SkeletonComms.REGS.RegTick)
                    {
                        tickChecked = true;
                    }
                }
        ...
        private int getRegs(F7SkeletonComms.REGS i)
        {
            UInt16 value = theBoard.GetReg(i);
            int result = value;
            if (F7SkeletonComms.RegSigned(i))
            {
                result = (Int16)value;
            }
            return result;
        ...
        private int[] getRegs(F7SkeletonComms.REGS[] i)
        {
            ...
            {
                if (F7SkeletonComms.RegSigned(i[j]))
                {
        ...
        F7SkeletonComms.REGS[] allRegs;

        private void initRegs()
        {
            statusList.Enabled = true;
            checkedCells = new List<F7SkeletonComms.REGS>();
            statusList.Rows.Clear();
            allRegs = new F7SkeletonComms.REGS[(int)F7SkeletonComms.REGS.RegLast];
            for (F7SkeletonComms.REGS i = (F7SkeletonComms.REGS)0; i < F7SkeletonComms.REGS.RegLast; i++)
            {
                allRegs[(int)i] = i;
            }
            ...
            statusList.Rows[(int)F7SkeletonComms.REGS.RegTick].Cells[2].ToolTipText = "ms";
        }
        ...
        List<F7SkeletonComms.REGS> checkedCells;
        ...
        private void statusList_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (e.RowIndex >= (int)F7SkeletonComms.REGS.RegLast)
                {
            ...
            {
                if (e.RowIndex < 0 || e.RowIndex >= (int)F7SkeletonComms.REGS.RegLast)
                {
                    return;
                }
                int newvalue;
                ...
                try
                {
                    if (F7SkeletonComms.RegSigned((F7SkeletonComms.REGS)e.RowIndex))
                    {
                        if (newvalue >= -32768 && newvalue <= 32767)
                        {
                            bool success = theBoard.SetReg((F7SkeletonComms.REGS)e.RowIndex, (ushort)newvalue);
                        }
                        newvalue = (int)((short)theBoard.GetReg((F7SkeletonComms.REGS)e.RowIndex));
                    }
                    else
                    {
                        ...
                        newvalue = (int)theBoard.GetReg((F7SkeletonComms.REGS)e.RowIndex);
                    }
        ...
        private void InitParams()
        {
            ...
            for (F7SkeletonComms.NVparams i = (F7SkeletonComms.NVparams)0; i < F7SkeletonComms.NVparams.NvLast; i++)
            {
        ...
        private void ReadParameters()
        {
            for (F7SkeletonComms.NVparams i = (F7SkeletonComms.NVparams)0; i < F7SkeletonComms.NVparams.NvLast; i++)
            {
                ...
                {
                    int value;
                    if (F7SkeletonComms.NvpSigned(i))
                    {
        ...
        private void parameters_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (F7SkeletonComms.NvpSigned((F7SkeletonComms.NVparams)e.RowIndex))
                {
                    if (value >= -32768 && value <= 32767)
                    {
                        bool success = theBoard.SetNvParam((F7SkeletonComms.NVparams)e.RowIndex, (ushort)value);
                    }
                    value = (int)((short)theBoard.GetNvParam((F7SkeletonComms.NVparams)e.RowIndex));
                }
                else
                {
                    ...
                    value = (int)theBoard.GetNvParam((F7SkeletonComms.NVparams)e.RowIndex);
                }
