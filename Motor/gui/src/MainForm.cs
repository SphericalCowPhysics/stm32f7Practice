...
namespace McuMotorNS {
    public partial class MainForm : Form
    {
        McuMotorComms theBoard;
        int fwRev = 0;
        ...
        private void InitBoard()
        {
            try
            {
                ...
                fwRev = theBoard.GetReg(McuMotorComms.REGS.RegFirmWareVersion);
            }
            catch (Exception)
            {
                fwRev = 0;
        ...
        private void PopulatePorts()
        {
            ...
            List<string> portNames = McuMotorComms.GetPortNames();
        ...
        {
            tickChecked = false;
            for (McuMotorComms.REGS i = (McuMotorComms.REGS)0; i < McuMotorComms.REGS.RegLast; i++)
            {
                if (statusList.Rows[(int)i].Cells[0].Value.ToString() == true.ToString())
                {
                    logFile.Write(i.ToString() + ',');
                    if (i == McuMotorComms.REGS.RegTick)
                    {
                        tickChecked = true;
                    }
                }
        ...
        private int getRegs(McuMotorComms.REGS i)
        {
            UInt16 value = theBoard.GetReg(i);
            int result = value;
            if (McuMotorComms.RegSigned(i))
            {
                result = (Int16)value;
            }
            return result;
        ...
        private int[] getRegs(McuMotorComms.REGS[] i)
        {
            ...
            {
                if (McuMotorComms.RegSigned(i[j]))
                {
        ...
        McuMotorComms.REGS[] allRegs;

        private void initRegs()
        {
            statusList.Enabled = true;
            checkedCells = new List<McuMotorComms.REGS>();
            statusList.Rows.Clear();
            allRegs = new McuMotorComms.REGS[(int)McuMotorComms.REGS.RegLast];
            for (McuMotorComms.REGS i = (McuMotorComms.REGS)0; i < McuMotorComms.REGS.RegLast; i++)
            {
                allRegs[(int)i] = i;
            }
            ...
            statusList.Rows[(int)McuMotorComms.REGS.RegTick].Cells[2].ToolTipText = "ms";
        }
        ...
        List<McuMotorComms.REGS> checkedCells;
        ...
        private void statusList_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (e.RowIndex >= (int)McuMotorComms.REGS.RegLast)
                {
            ...
            {
                if (e.RowIndex < 0 || e.RowIndex >= (int)McuMotorComms.REGS.RegLast)
                {
                    return;
                }
                int newvalue;
                ...
                try
                {
                    if (McuMotorComms.RegSigned((McuMotorComms.REGS)e.RowIndex))
                    {
                        if (newvalue >= -32768 && newvalue <= 32767)
                        {
                            bool success = theBoard.SetReg((McuMotorComms.REGS)e.RowIndex, (ushort)newvalue);
                        }
                        newvalue = (int)((short)theBoard.GetReg((McuMotorComms.REGS)e.RowIndex));
                    }
                    else
                    {
                        ...
                        newvalue = (int)theBoard.GetReg((McuMotorComms.REGS)e.RowIndex);
                    }
        ...
        private void InitParams()
        {
            ...
            for (McuMotorComms.NVparams i = (McuMotorComms.NVparams)0; i < McuMotorComms.NVparams.NvLast; i++)
            {
        ...
        private void parameters_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (McuMotorComms.NvpSigned((McuMotorComms.NVparams)e.RowIndex))
                {
                    if (value >= -32768 && value <= 32767)
                    {
                        bool success = theBoard.SetNvParam((McuMotorComms.NVparams)e.RowIndex, (ushort)value);
                    }
                    value = (int)((short)theBoard.GetNvParam((McuMotorComms.NVparams)e.RowIndex));
                }
                else
                {
                    ...
                    value = (int)theBoard.GetNvParam((McuMotorComms.NVparams)e.RowIndex);
                }
