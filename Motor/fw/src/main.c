...
int main(void)
{
	...
	uint32_t e, q1, q2, qpol, qq1, qq2, qqpol = 0;
	uint32_t cw, ccw = 0;
	int totalRot = 0;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	{
		Regs.u16[RegEncoderCCwSteps]++;			//hit on rising, falling, rising/falling
	}
	*/

	//Trigger PG6 on rising edge, and ask if PG7 is high or low
	/*
	if (GPIO_Pin == GPIO_PIN_6)
	{
		...
		else if (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_7) == 1)
		{
	...
	/* //We can now hit both PG6&7 interrupts but only when the other pin is LOW*/
	...
	if (GPIO_Pin == GPIO_PIN_6)
	{
		...
		if ((HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_6) == 0) && (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_7) == 0))	//hit on PG6 rising/falling, rising?!?, falling. Even if PG7 is tied to Encoder Out B, or Grnd, or 3.3V, or 5V.
		{r++;}
		if ((HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_6) == 0) && (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_7) == 1))	//never
		{r++;}
		if ((HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_6) == 1) && (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_7) == 0))	//never
		{r++;}
		if ((HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_6) == 1) && (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_7) == 1))	//never
		{r++;}		
	}
	else	//guarantee PG6 is the only pin causing interrupts
	{
		asm("bkpt 255");
	}
