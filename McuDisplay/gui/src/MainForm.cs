...
namespace McuDisplayNS {
    public partial class MainForm : Form
    {
        McuDisplayComms theBoard;
        int fwRev = 0;
        ...
        private void InitBoard()
        {
            try
            {
                ...
                fwRev = theBoard.GetReg(McuDisplayComms.REGS.RegFirmWareVersion);
            }
            catch (Exception)
            {
                fwRev = 0;
        ...
        private void PopulatePorts()
        {
            ...
            List<string> portNames = McuDisplayComms.GetPortNames();
        ...
        {
            tickChecked = false;
            for (McuDisplayComms.REGS i = (McuDisplayComms.REGS)0; i < McuDisplayComms.REGS.RegLast; i++)
            {
                if (statusList.Rows[(int)i].Cells[0].Value.ToString() == true.ToString())
                {
                    logFile.Write(i.ToString() + ',');
                    if (i == McuDisplayComms.REGS.RegTick)
                    {
                        tickChecked = true;
                    }
                }
        ...
        private int getRegs(McuDisplayComms.REGS i)
        {
            UInt16 value = theBoard.GetReg(i);
            int result = value;
            if (McuDisplayComms.RegSigned(i))
            {
                result = (Int16)value;
            }
            return result;
        ...
        private int[] getRegs(McuDisplayComms.REGS[] i)
        {
            ...
            {
                if (McuDisplayComms.RegSigned(i[j]))
                {
        ...
        McuDisplayComms.REGS[] allRegs;

        private void initRegs()
        {
            statusList.Enabled = true;
            checkedCells = new List<McuDisplayComms.REGS>();
            statusList.Rows.Clear();
            allRegs = new McuDisplayComms.REGS[(int)McuDisplayComms.REGS.RegLast];
            for (McuDisplayComms.REGS i = (McuDisplayComms.REGS)0; i < McuDisplayComms.REGS.RegLast; i++)
            {
                allRegs[(int)i] = i;
            }
            ...
            statusList.Rows[(int)McuDisplayComms.REGS.RegTick].Cells[2].ToolTipText = "ms";
        }
        ...
        List<McuDisplayComms.REGS> checkedCells;
        ...
        private void statusList_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (e.RowIndex >= (int)McuDisplayComms.REGS.RegLast)
                {
            ...
            {
                if (e.RowIndex < 0 || e.RowIndex >= (int)McuDisplayComms.REGS.RegLast)
                {
                    return;
                }
                int newvalue;
                ...
                try
                {
                    if (McuDisplayComms.RegSigned((McuDisplayComms.REGS)e.RowIndex))
                    {
                        if (newvalue >= -32768 && newvalue <= 32767)
                        {
                            bool success = theBoard.SetReg((McuDisplayComms.REGS)e.RowIndex, (ushort)newvalue);
                        }
                        newvalue = (int)((short)theBoard.GetReg((McuDisplayComms.REGS)e.RowIndex));
                    }
                    else
                    {
                        ...
                        newvalue = (int)theBoard.GetReg((McuDisplayComms.REGS)e.RowIndex);
                    }
        ...
        private void InitParams()
        {
            ...
            for (McuDisplayComms.NVparams i = (McuDisplayComms.NVparams)0; i < McuDisplayComms.NVparams.NvLast; i++)
            {
        ...
        private void ReadParameters()
        {
            for (McuDisplayComms.NVparams i = (McuDisplayComms.NVparams)0; i < McuDisplayComms.NVparams.NvLast; i++)
            {
                ...
                {
                    int value;
                    if (McuDisplayComms.NvpSigned(i))
                    {
        ...
        private void parameters_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                if (McuDisplayComms.NvpSigned((McuDisplayComms.NVparams)e.RowIndex))
                {
                    if (value >= -32768 && value <= 32767)
                    {
                        bool success = theBoard.SetNvParam((McuDisplayComms.NVparams)e.RowIndex, (ushort)value);
                    }
                    value = (int)((short)theBoard.GetNvParam((McuDisplayComms.NVparams)e.RowIndex));
                }
                else
                {
                    ...
                    value = (int)theBoard.GetNvParam((McuDisplayComms.NVparams)e.RowIndex);
                }
